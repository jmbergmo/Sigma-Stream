  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in duration-500">
          ...
          <ol className="space-y-4 mb-8">
            {[
              ...
              <li key={idx} className="flex gap-3 text-sm text-slate-600 dark:text-slate-400">
                <span className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold border border-slate-200 dark:border-slate-600 shadow-sm">{idx + 1}</span>
                <span><strong className="block text-slate-800 dark:text-slate-200">{step.title}</strong>{step.desc}</span>
              </li>
            ))}
          </ol>
          <div className="border-t border-slate-100 dark:border-slate-700 pt-6">
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-2 uppercase tracking-wide">About DFSS & DOE Optimization</h3>
            <div className="mb-4 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm">
              <img src="/assets/sigma-stream-demo.webp" alt="Sigma Stream Demo" className="w-full h-auto" />
            </div>
      ...
      <div className="lg:col-span-8 space-y-6">
        <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Input Variables</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Define factors and their experimental levels.</p>
            </div>
            {currentUser && (
              <div className="flex gap-2">
                <button
                  onClick={() => setIsSnapshotModalOpen(true)}
                  className="text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                >
                ...
                <button
                  ...
                  className="text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                >
          ...
          <div className="space-y-4 mb-8">
            <div className="grid grid-cols-12 gap-3 px-4 py-2 bg-slate-100/50 dark:bg-slate-900/50 rounded-t-lg border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
            ...
            <div className="space-y-3">
              {factors.map((factor) => (
                <div key={factor.id} className="grid grid-cols-12 gap-3 items-center bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm transition-all group/row">
                  {/* Name */}
                  <div className="col-span-3">
                    <input type="text" value={factor.name} onChange={(e) => updateFactorName(factor.id, e.target.value)} className="w-full text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none placeholder:font-normal" placeholder="e.g. Temp" />
                  </div>
                    ...
                    if (isPresent) {
                      return (
                        <div key={idx} className="col-span-2 relative group/cell">
                          <span className="absolute left-2 top-2 text-[10px] text-slate-400 dark:text-slate-500 font-bold pointer-events-none">L{idx + 1}</span>
                          <input
                            type="number"
                            value={isNaN(factor.levels[idx]) ? '' : factor.levels[idx]}
                            onChange={(e) => updateFactorLevel(factor.id, idx, e.target.value)}
                            className="w-full text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-2 py-2 pl-7 focus:ring-2 focus:ring-blue-500 outline-none text-slate-600 dark:text-slate-300 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          />
                          ...
                          {idx >= 2 && idx === factor.levels.length - 1 && (
                            <button
                              ...
                              className="absolute right-1 top-1.5 p-1 text-slate-300 dark:text-slate-600 hover:text-red-500 opacity-0 group-hover/cell:opacity-100 transition-opacity"
                      ...
                      return (
                        <div key={idx} className="col-span-2 relative">
                          <button onClick={() => addLevelToFactor(factor.id)} className="w-full h-[38px] border-2 border-dashed border-slate-200 dark:border-slate-700 rounded text-xs font-bold text-slate-400 dark:text-slate-500 hover:border-blue-300 dark:hover:border-blue-700 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all flex items-center justify-center gap-1">
                  ...
                  <div className="col-span-1 flex justify-end">
                    <button onClick={() => removeFactor(factor.id)} className="text-slate-300 dark:text-slate-600 hover:text-red-500 p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors">
          ...
          <div className="flex items-center justify-end gap-4 pt-4 border-t border-slate-100 dark:border-slate-700">
            <div className="text-xs text-slate-400 dark:text-slate-500">{factors.reduce((acc, f) => acc * f.levels.length, 1)} Runs will be generated</div>

            <button
              onClick={onClear}
              className="px-4 py-2.5 rounded-xl font-bold text-slate-600 dark:text-slate-300 hover:text-red-700 dark:hover:text-red-400 bg-slate-50 dark:bg-slate-900 hover:bg-red-50/50 dark:hover:bg-red-900/50 border border-slate-300 dark:border-slate-600 hover:border-red-200 dark:hover:border-red-800 shadow-sm transition-all flex items-center gap-2"
